...
- **Never `from app import ...` (or `import app`) at a test module's TOP level.** It loads the real app into `sys.modules` at collection time and defeats the conftest isolation, re-breaking the stub-based tests. Use the `app`/`client` fixtures, or import app modules lazily *inside* test functions (see `tests/test_app_factory.py`).
